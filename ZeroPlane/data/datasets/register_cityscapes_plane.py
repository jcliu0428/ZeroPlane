...
def load_single_cityscapes_plane_json(json_file):
    """
    Args:
...
def register_single_cityscapes_plane_annos_seg(name,  metadata, plane_seg_json):
    plane_seg_name = "single_" + name

    DatasetCatalog.register(
        plane_seg_name,
        lambda: load_single_cityscapes_plane_json(plane_seg_json),
    )
    ...
    MetadataCatalog.get(plane_seg_name).set(
        ...
        evaluator_type = "cityscapes_plane_seg",
        ignore_label = 20,
        # **metadata,
    )


def register_all_single_cityscapes_plane_annos_seg(json_root):

    for split in ['val']:
        name = "cityscapes_plane_seg" + "_" + split
        num = VAL_NUM
        plane_seg_json = pjoin(json_root, "cityscapes_plane_len" + str(num) + "_" + split + ".json")

        register_single_cityscapes_plane_annos_seg(
            name,
            # get_metadata(),
...
_root = os.path.join(os.getenv("DETECTRON2_DATASETS", "datasets"), 'cityscapes_plane') # 26
register_all_single_cityscapes_plane_annos_seg(_root)
