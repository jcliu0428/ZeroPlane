...
def load_single_oasis_plane_json(json_file):
    """
    Args:
...
def register_single_oasis_plane_annos_seg(name,  metadata, plane_seg_json):
    plane_seg_name = "single_" + name

    DatasetCatalog.register(
        plane_seg_name,
        lambda: load_single_oasis_plane_json(plane_seg_json),
    )
    ...
    MetadataCatalog.get(plane_seg_name).set(
        ...
        evaluator_type = "oasis_plane_seg",
        ignore_label = 20,
        # **metadata,
    )


def register_all_single_oasis_plane_annos_seg(json_root):

    for split in ['val']:
        name = "oasis_plane_seg" + "_" + split
        num = VAL_NUM
        plane_seg_json = pjoin(json_root, "oasis_plane_len" + str(num) + "_" + split + ".json")

        register_single_oasis_plane_annos_seg(
            name,
...
_root = os.path.join(os.getenv("DETECTRON2_DATASETS", "datasets"), 'oasis_plane')
register_all_single_oasis_plane_annos_seg(_root)
...
if __name__ == "__main__":
    plot_samples("single_oasis_plane_seg_test", n=3)
