...
def load_single_replica_hm3d_plane_json(json_file):
    """
    Args:
...
def register_single_replica_hm3d_plane_annos_seg(name,  metadata, plane_seg_json):

    plane_seg_name = "single_" + name

    DatasetCatalog.register(
        plane_seg_name,
        lambda: load_single_replica_hm3d_plane_json(plane_seg_json),
    )
    ...
    MetadataCatalog.get(plane_seg_name).set(
        ...
        evaluator_type = "replica_hm3d_plane_seg",
        ignore_label = MAX_NUM_PLANES,
        # **metadata,
    )


def register_all_single_replica_hm3d_plane_annos_seg(json_root):

    for split in ['train', 'val']:
        name = "replica_hm3d_plane_seg" + "_" + split
        num = TRAIN_NUM if split == 'train' else VAL_NUM
        plane_seg_json = pjoin(json_root, "replica_hm3d_plane_len" + str(num) + "_" + split + ".json")

        register_single_replica_hm3d_plane_annos_seg(
            name,
...
_root = os.path.join(os.getenv("DETECTRON2_DATASETS", "datasets"), 'replica_hm3d_plane')
register_all_single_replica_hm3d_plane_annos_seg(_root)
...
if __name__ == "__main__":
    plot_samples("single_replica_hm3d_plane_seg_train", n=3)
