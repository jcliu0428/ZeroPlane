...
def load_single_outdoor_mixed_plane_json(json_file):
    """
    Args:
...
def register_single_outdoor_mixed_plane_annos_seg(name,  metadata, plane_seg_json):

    plane_seg_name = "single_" + name

    DatasetCatalog.register(
        plane_seg_name,
        lambda: load_single_outdoor_mixed_plane_json(plane_seg_json),
    )
...
def register_all_single_outdoor_mixed_plane_annos_seg(json_root):

    for split in ['train', 'val']:
        name = "outdoor_mixed_plane_seg" + "_" + split

        if split == 'train':
            if with_sanpo:
                plane_seg_json = pjoin(json_root, 'new_outdoor_mixed_plane_len' + str(TRAIN_NUM) + '_' + split + '.json')

            else:
                plane_seg_json = pjoin(json_root, "syn_vkitti_mixed_plane_len" + str(TRAIN_NUM) + "_" + split + ".json")

        elif split == 'val':
            plane_seg_json = pjoin(osp.dirname(json_root), "parallel_domain_plane/parallel_domain_plane_len" + str(VAL_NUM) + "_" + split + ".json")

        register_single_outdoor_mixed_plane_annos_seg(
            name,
...
register_all_single_outdoor_mixed_plane_annos_seg(_root)
...
if __name__ == "__main__":
    ...
    plot_samples("single_outdoor_mixed_plane_seg_train", n = 3)
