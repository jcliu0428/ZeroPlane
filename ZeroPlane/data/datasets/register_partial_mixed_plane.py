...
def load_single_partial_mixed_plane_json(json_file):
    """
    Args:
...
def register_single_partial_mixed_plane_annos_seg(name,  metadata, plane_seg_json):

    plane_seg_name = "single_" + name

    DatasetCatalog.register(
        plane_seg_name,
        lambda: load_single_partial_mixed_plane_json(plane_seg_json),
    )
...
def register_all_single_partial_mixed_plane_annos_seg(json_root):

    for split in ['train', 'val']:
        name = "partial_mixed_plane_seg" + "_" + split
        num = TRAIN_NUM if split == 'train' else VAL_NUM

        if split == 'train':
            plane_seg_json = pjoin(json_root, "scannet_syn_mixed_plane_len" + str(num) + "_" + split + ".json")

        elif split == 'val':
            if eval_indoor:
                plane_seg_json = pjoin(osp.dirname(json_root), "scannetv1_plane/scannetv1_plane_len" + str(num) + "_" + split + ".json")
            else:
                plane_seg_json = pjoin(osp.dirname(json_root), "synthia_plane/syn_plane_len" + str(num) + "_" + split + ".json")

            # plane_seg_json = pjoin(osp.dirname(json_root), "mp3d_plane/mp3d_plane_len" + str(num) + "_" + split + ".json")

        register_single_partial_mixed_plane_annos_seg(
            name,
...
register_all_single_partial_mixed_plane_annos_seg(_root)
