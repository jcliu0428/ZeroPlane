...
def load_single_indoor_mixed_plane_json(json_file):
    """
    Args:
...
def register_single_indoor_mixed_plane_annos_seg(name,  metadata, plane_seg_json):

    plane_seg_name = "single_" + name

    DatasetCatalog.register(
        plane_seg_name,
        lambda: load_single_indoor_mixed_plane_json(plane_seg_json),
    )
...
def register_all_single_indoor_mixed_plane_annos_seg(json_root):

    for split in ['train', 'val']:
        name = "indoor_mixed_plane_seg" + "_" + split
        num = TRAIN_NUM if split == 'train' else VAL_NUM

        if split == 'train':
            plane_seg_json = pjoin(json_root, "scannet_mp3d_mixed_plane_len" + str(num) + "_" + split + ".json")
        ...
        register_single_indoor_mixed_plane_annos_seg(
            name,
...
register_all_single_indoor_mixed_plane_annos_seg(_root)
...
if __name__ == "__main__":
    ...
    plot_samples("single_indoor_mixed_plane_seg_train", n = 3)
