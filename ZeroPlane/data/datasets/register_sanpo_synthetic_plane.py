...
def load_single_sanpo_synthetic_plane_json(json_file):
    """
    Args:
...
def register_single_sanpo_synthetic_plane_annos_seg(name,  metadata, plane_seg_json):

    plane_seg_name = "single_" + name

    DatasetCatalog.register(
        plane_seg_name,
        lambda: load_single_sanpo_synthetic_plane_json(plane_seg_json),
    )
    ...
    MetadataCatalog.get(plane_seg_name).set(
        ...
        evaluator_type = "sanpo_synthetic_plane_seg",
        ignore_label = MAX_NUM_PLANES,
        # **metadata,
    )


def register_all_single_sanpo_synthetic_plane_annos_seg(json_root):

    for split in ['train', 'val']:
        name = "sanpo_synthetic_plane_seg" + "_" + split

        if split == 'train':
            plane_seg_json = os.path.join(json_root, "sanpo_synthetic_plane_len" + str(TRAIN_NUM) + "_" + split + ".json")

        else:
            plane_seg_json = os.path.join(json_root, 'sanpo_synthetic_plane_len' + str(VAL_NUM) + '_' + split + '.json')

        # else:
        #     plane_seg_json = os.path.join('./datasets/parallel_domain_plane/parallel_domain_plane_len356_val.json')

        register_single_sanpo_synthetic_plane_annos_seg(
            name,
...
_root = os.path.join(os.getenv("DETECTRON2_DATASETS", "datasets"), 'sanpo_synthetic_plane')  # 26
register_all_single_sanpo_synthetic_plane_annos_seg(_root)
...
if __name__ == "__main__":
    ...
    plot_samples("single_sanpo_synthetic_plane_seg_train", n = 3)
