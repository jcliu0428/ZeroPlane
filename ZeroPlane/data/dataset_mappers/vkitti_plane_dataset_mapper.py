...
__all__ = ['SingleVKITTIPlaneDatasetMapper']
...
class SingleVKITTIPlaneDatasetMapper():
    """
    ...
    def __init__(
        ...
        *,
        ...
        logging.getLogger(__name__).info(
            "[SingleVKITTIPlaneDatasetMapper] Full TransformGens used in training: {}".format(
                str(self.tfm_gens)
            )
        )
        ...
        if self.mix_anchor:
            ...
            self.anchor_normals = np.load('./cluster_anchor/vkitti_normal_anchors_{}.npy'.format(normal_class_num))
            self.anchor_offsets = np.load('./cluster_anchor/vkitti_offset_anchors_{}.npy'.format(offset_class_num))
